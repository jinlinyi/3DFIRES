...
def camera_mesh_from_fov_camera(fov_camera, scale=0.1, color=None):
    """
    return pytorch3d camera mesh from pytorch3d camera.
    """
    radius = 0.02
    ...
    for k in range(len(edges)):
        ...
        cam_verts.extend(cyl_verts)
        cam_inds.extend(cyl_ind)
    cam_verts = torch.tensor(np.array(cam_verts), dtype=torch.float32)
    cam_inds = torch.tensor(np.array(cam_inds).astype(np.int64), dtype=torch.int64)

    cam_verts = (
        fov_camera.get_world_to_view_transform().inverse().transform_points(cam_verts)
    )

    if color is None:
        color = torch.tensor([[1.0, 0.0, 0.0]])
    else:
        color = torch.tensor(color[None])
    vertex_colors = color.expand(cam_verts.shape)
    textures = TexturesVertex(verts_features=[vertex_colors])

    return Meshes(verts=[cam_verts], faces=[cam_inds], textures=textures)
